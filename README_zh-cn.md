...
- **自定义主题**：支持主题安装与配置。本 fork 已移除插件及 JavaScript 自定义通知，说明见 [FORK.md](FORK.md)。

## 快速开始

安装本 fork 请使用 [FORK.md](FORK.md) 和 [deploy/compose.yaml](deploy/compose.yaml)，指定 `ghcr.io/mghts/komari` 的明确版本。以下上游应用商店和安装文档不会安装本 fork。
