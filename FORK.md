...
Server `1.4.8` 的内置前端使用 Agent `1.2.63`，替代上述 `1.2.62` 的重试限制。Linux 一键命令可直接重复执行，按当前命令覆盖程序、节点地址/token、配置和服务参数，无需先卸载。未指定的选项恢复安装器默认值；Web 仍默认启用自动更新。省略 endpoint/token 的升级方式继续保留现有配置。Agent 自更新仅接受更高的正式版，避免重装较新版本后被降级。

安装器不再生成备份目录，也不会清理历史备份。旧文件只在本次安装进程内暂存，正常捕获到启动失败等错误时尝试恢复；强制结束进程或断电后没有持久备份可供自动恢复。发布检查覆盖重复安装、更换节点及选项、旧配置字段清除、配置权限、无备份目录和启动失败恢复。升级 Server 后重新从 Web 复制命令，确认下载版本为 `1.2.63`。

## Server 1.4.9 升级说明

`1.4.9` 修复 Agent 基础信息越权更新、跨节点上报、无界请求体、密码存储与登录尝试限制、备份恢复和主题替换失败时的数据保护、主题下载边界、网页终端并发，以及后台日志分页。配套 Agent 仍为 `1.2.63`，现有节点无需因本次 Server 升级重装。

升级前按上文确认实际数据挂载，停止 Server 后对完整数据目录做一致性备份，并记录原镜像或二进制版本。Docker Compose 用户将部署目录 `.env` 中的 `KOMARI_VERSION` 改为 `1.4.9`，再执行 `docker compose pull`、`docker compose up -d` 和登录、节点、历史指标检查。systemd 用户备份数据后，使用本 fork 的 `install-komari.sh` 选择升级并输入明确版本 `1.4.9`。

旧账号密码在首次成功登录时升级为 Argon2id 哈希。旧版 Server 无法验证已升级的哈希；若需要回退到 `1.4.8`，必须同时恢复升级前的一致性数据备份，不能只替换镜像或二进制。登录接口增加短时限流，普通请求体上限为 1 MiB；分片上传和图标上传保留各自较大的限额。容器测试已覆盖安装、登录、Agent 上报、远程命令、重启和持久化；真实 VPS 的网页终端与多磁盘指标仍需部署后验收。
